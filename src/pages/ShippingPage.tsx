...
export default function ShippingPage() {
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold">إدارة الشحنات</h3>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <Calendar className="w-3.5 h-3.5" /> شحن: {s.ship_date}
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <Calendar className="w-3.5 h-3.5" /> وصول: {s.expected_arrival_date}
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <DollarSign className="w-3.5 h-3.5" /> ${s.shipping_cost.toLocaleString()}
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <Package className="w-3.5 h-3.5" /> {s.cartons_count} كرتون
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                <Weight className="w-3.5 h-3.5" /> {s.weight} كغ
              </div>
              <div className="flex items-center gap-1.5 text-muted-foreground">
                {s.shipping_type === 'sea' ? '🚢 بحري' : '✈️ جوي'}
              </div>
            </div>
          </motion.div>
        ))}
